...
const FilterBar = ({ filters, setFilters, products }) => {
  ...
  const availableBrands = [...new Set(products.map(product => product.brand || product.make).filter(Boolean))];
    ...
    <div className="space-y-4">
      ...
      {showFilters && (
        <div className="bg-card rounded-lg p-6 border border-secondary space-y-6">
          ...
          {/* Price range */}
          <div className="space-y-3">
            <Label>Price Range: LKR {filters.priceRange[0].toLocaleString()} - LKR {filters.priceRange[1].toLocaleString()}</Label>
            <Slider
              min={0}
              max={500000}
              step={1000}
