...
5. **Same database as psql** – The backend must connect to the **same database** where you see the data. If you run `SELECT * FROM branches` in psql and see rows (e.g. `IphoneCenterDB=#`), then `DATABASE_URL` must use that database name. Example: `postgresql://user:pass@host:5432/IphoneCenterDB` (the part after the last `/` is the database name). If you use `pos_system` or another name, the backend will query a different DB where `branches` may be empty or missing.
6. **Permission denied (42501)** – If the schema was run as `postgres` (or another user) but the app connects with a different user (from `DATABASE_URL`), that user may lack SELECT on `branches`. Connect to PostgreSQL as a superuser and run (replace `your_app_user` with the username from your `DATABASE_URL`):
   ```sql
